...
process.env.PI_FEATURE_FLOW_ROOT ??= mkdtempSync(
	join(tmpdir(), "pi-feature-flow-test-"),
);
...
const load = () =>
	Effect.runPromise(
		Effect.map(FeatureConfig, (service) => service.config).pipe(
			Effect.provide(FeatureConfig.Default),
		),
	);
...
describe("main session model ownership", () => {
	it("does not let feature flow change the active Pi model or thinking level", () => {
		const extension = readFileSync(
			new URL("../extensions/feature-flow.ts", import.meta.url),
			"utf8",
		);
...
describe("feature config", () => {
	it("provides complete defaults without a config file", async () => {
		...
		assert.deepEqual(config.routes.worker, {
			model: "openai-codex/gpt-5.6-sol",
			thinking: "low",
		});
		assert.deepEqual(config.routes.fableWorker, {
		...
		assert.deepEqual(config.routes.validator, {
			model: "openai-codex/gpt-5.6-sol",
			thinking: "high",
		});
		assert.deepEqual(
			Object.keys(config.routes).sort((left, right) =>
				left.localeCompare(right),
			),
			["adversary", "fableWorker", "validator", "worker"],
		);
		assert.deepEqual(config.routes.adversary, {
			model: "anthropic/claude-fable-5",
			thinking: "high",
		});
	...
	it("routes adversarial review to Sol high when Fable 5 authored the plan", async () => {
		const config = await load();
		assert.equal(
		...
		assert.equal(
			isFableFiveModel({ provider: "openai-codex", id: "gpt-5.6-sol" }),
			false,
		);
		assert.deepEqual(
			adversaryRoute(config, { provider: "anthropic", id: "claude-fable-5" }),
			{ model: "openai-codex/gpt-5.6-sol", thinking: "high" },
		);
		assert.deepEqual(
			adversaryRoute(config, { provider: "openai-codex", id: "gpt-5.6-sol" }),
	...
	it("overrides selectively from config.json", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				...
				turnSnapshot: "off",
				archive: {
					repository: "private-feature-context",
					extraPaths: ["reports/{featureId}.md"],
				},
			}),
		);
		const config = await load();
