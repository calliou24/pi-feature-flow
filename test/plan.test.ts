...
describe("plan artifact gate", () => {
	const artifact = {
		url: "https://ubuntu-desktop.example.ts.net/feature-plans/f/plan-rev4.md",
		publishedAt: "now",
		planRevision: 4,
	...
	it("requires an approved checkpoint for implementation", () => {
		...
		assert.ok(
			implementationProblem(
				{ kind: "plan", status: "pending", updatedAt: now },
				PLAN,
				artifact,
			),
		);
		assert.equal(
			implementationProblem(
				{ kind: "plan", status: "approved", updatedAt: now },
				PLAN,
				artifact,
			),
			null,
	...
	it("allows initial and pending-checkpoint plan publication", () => {
		const now = new Date().toISOString();
		const planning = {
			version: 3,
			featureId: "f",
			title: "Test",
			workItem: { kind: "feature", key: "F-1", source: "F-1" },
			project: { cwd: "/tmp" },
			status: "planning",
		...
		assert.equal(planPublicationProblem(planning), null);
		assert.equal(
			planPublicationProblem({
				...planning,
				status: "awaiting-approval",
				checkpoint: { kind: "plan", status: "pending", updatedAt: now },
			}),
			null,
		);
	});

	it("rejects stale plan publication after approval or during execution", () => {
		const now = new Date().toISOString();
		const approved = {
			version: 3,
			featureId: "f",
			...
			updatedAt: now,
			revision: 1,
			checkpoint: { kind: "plan", status: "approved", updatedAt: now },
			planArtifact: null,
			sessions: [],
			executionLease: null,
			lastKnownGit: null,
		...
		assert.match(planPublicationProblem(approved) ?? "", /approved/);
		assert.match(
			planPublicationProblem({
				...approved,
				status: "implementing",
		...
		assert.match(
			...
			/validation is being started/,
		);
	});

	it("accepts Tailscale HTTPS and local file URLs", () => {
		assert.equal(
			extractArtifactUrl("published file:///home/x/plan-rev4.md ok"),
			"file:///home/x/plan-rev4.md",
		);
		assert.equal(
			extractArtifactUrl(
				"see https://ubuntu-desktop.example.ts.net/feature-plans/f/plan-rev4.md",
			),
			"https://ubuntu-desktop.example.ts.net/feature-plans/f/plan-rev4.md",
		);
		assert.equal(
			extractArtifactUrl("see https://public.example.com/plan.md"),
			null,
		);
...
describe("validation verdict", () => {
	it("reads PASS from the last assistant message in a jsonl session", () => {
		const jsonl = [
			...
			JSON.stringify({
				message: {
					role: "assistant",
					content: [{ type: "text", text: "PASS — no blockers." }],
				},
			}),
		].join("\n");
	...
	it("handles plain-text transcripts", () => {
		const verdict = validationVerdictFromContents([
...
describe("continuation context", () => {
	it("composes a bounded handoff", () => {
		...
		const text = composeContinuationContext(
			state,
			{ assumptions: "a", decisions: "d", plan: "p", threadLog: "t" },
			{ status: "", log: "abc123 init" },
		);
