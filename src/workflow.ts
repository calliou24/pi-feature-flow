  ...
  effect: Effect.gen(function* () {
    ...
    const publishGeneratedPlan = (featureId: string) =>
      Effect.gen(function* () {
        ...
        yield* store.update(featureId, (draft) => {
          draft.planArtifact = { url, publishedAt: new Date().toISOString(), planRevision, planHash: planHash(plan) };
          draft.activeStage = "planning";
          draft.status = "awaiting-approval";
          draft.checkpoint = { kind: "plan", status: "pending", updatedAt: new Date().toISOString(), note: url };
          draft.lastError = null;
        });
        yield* store.appendLedger(featureId, { type: "plan.published", url, planRevision });
        return url;
      });
    ...
    const decideCheckpoint = (featureId: string, decision: "approve" | "reject", note: string, piSessionId: string | null) =>
      Effect.gen(function* () {
        ...
        if (state.checkpoint.status !== "pending") {
          return yield* Effect.fail(new StageNotReady({ reason: `No pending checkpoint to ${decision}.` }));
        }
        if (decision === "approve" && state.checkpoint.kind === "plan") {
          const plan = yield* store.readArtifact(featureId, "plan");
          const problem = planArtifactProblem(plan, state.planArtifact);
          if (problem) return yield* Effect.fail(new StageNotReady({ reason: problem }));
        }
        const updated = yield* store.update(featureId, (draft) => {
          ...
          if (decision === "approve" && draft.checkpoint.kind === "plan") draft.status = "planned";
          if (decision === "reject") draft.status = "blocked";
        });
        yield* store.appendLedger(featureId, { type: "checkpoint.decided", decision, note, piSessionId });
        return updated;
      });
    ...
    const spawnStage = (featureId: string, stage: RunStage, task: string, cwd: string | null, parentSessionId: string | null, workerKind: WorkerKind = "sol", packages?: readonly string[]) =>
      Effect.gen(function* () {
        ...
        const running = state.sessions.find((session) => session.kind === "subagent" && !session.endedAt && (session.stage === "implementation" || session.stage === "validation"));
        if (running) return yield* Effect.fail(new StageNotReady({ reason: `Cannot start ${stage}; ${running.stage} run ${running.runId ?? "unknown"} is still active.` }));
        ...
        if (needsLease) {
          yield* store.reserveExecution(
            featureId,
            { token: requestId, stage, ownerSessionId: parentSessionId, createdAt: new Date().toISOString() },
            (current) =>
              Effect.gen(function* () {
                if (stage === "implementation") {
                  const plan = yield* store.readArtifact(featureId, "plan").pipe(Effect.orElseSucceed(() => ""));
                  const problem = implementationProblem(current.checkpoint, plan, current.planArtifact);
                  if (problem) return yield* Effect.fail(new Error(problem));
                }
                if (stage === "validation") {
                  const completed = current.sessions.some((session) =>
                    session.kind === "subagent" && session.stage === "implementation" && session.endedAt && ["complete", "completed"].includes(session.endReason ?? ""));
                  if (!completed) return yield* Effect.fail(new Error("Validation requires a completed implementation run."));
                }
              }),
          );
        ...
        const spawnResult = yield* spawnEffect.pipe(
          Effect.tapError((error: SpawnFailed) =>
            ...
            needsLease && error.outcome === "definitive"
              ? store.releaseExecution(featureId, requestId, false).pipe(Effect.ignore)
              : Effect.void
          ),
        );

        const record: SessionRecord = {
          kind: "subagent", sessionId: null, sessionFile: null, transcriptPath: null, cwd: runCwd,
          stage: stage === "adversary" ? "review" : stage, role: stageRole(stage),
          runId: spawnResult.runId, asyncDir: spawnResult.asyncDir, parentSessionId,
          startedAt: new Date().toISOString(), endedAt: null, endReason: null,
        };
        yield* store.update(featureId, (draft) => {
          if (needsLease && draft.executionLease?.token !== requestId) {
            throw new Error("Execution reservation changed before spawn persistence.");
          }
          if (needsLease) draft.executionLease = null;
          if (stage === "implementation" || stage === "validation") {
            draft.activeStage = stage;
            draft.status = stage === "implementation" ? "implementing" : "validating";
          }
    ...
    const completeRun = (
      ...
      Effect.gen(function* () {
        ...
        const feature = features.find((candidate) => candidate.sessions.some((session) => session.kind === "subagent" && session.runId === runId && !session.endedAt));
        if (!feature) return null;
        const index = feature.sessions.findIndex((session) => session.kind === "subagent" && session.runId === runId && !session.endedAt);
        const completedStage = feature.sessions[index]!.stage;
        const succeeded = ["complete", "completed"].includes(result?.status ?? "");
        ...
        const state = yield* store.update(feature.featureId, (draft) => {
          ...
          session.transcriptPath = result?.transcriptPath ?? session.transcriptPath;
          session.endReason = result?.status ?? "complete";
          if (session.stage === "implementation") {
            draft.status = succeeded ? "planned" : "blocked";
            draft.lastError = succeeded ? null : `Implementation run ${runId} ended without a confirmed successful status (${result?.status ?? "unknown"}).`;
            ...
            draft.lastError = passed ? null : (verdict?.summary || `Validation run ${runId} did not report PASS.`);
          }
        });
        yield* store.appendLedger(feature.featureId, {
          ...
          runId,
          asyncDir,
          result: result ? { status: result.status, agent: result.agent, sessionPath: result.sessionPath, transcriptPath: result.transcriptPath } : null,
