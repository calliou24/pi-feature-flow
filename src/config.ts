...
export const FEATURE_FLOW_ROOT =
	process.env.PI_FEATURE_FLOW_ROOT ??
	join(homedir(), ".pi", "agent", "feature-flow");
export const FEATURES_ROOT = join(FEATURE_FLOW_ROOT, "features");
export const CONFIG_PATH = join(FEATURE_FLOW_ROOT, "config.json");

const ThinkingLevel = Schema.Literal(
	"off",
	"minimal",
	"low",
	"medium",
...
export const SOL_HIGH_ADVERSARY_ROUTE: ModelRoute = {
...
export function isFableFiveModel(
	...
	return /fable[-_. ]?5(?:\b|[-_.])/i.test(identity);
}

const Routes = Schema.Struct({
	/** Default implementation subagent. */
	worker: Schema.optionalWith(ModelRoute, {
		default: () => ({
			model: "openai-codex/gpt-5.6-sol",
	...
	fableWorker: Schema.optionalWith(ModelRoute, {
		default: () => ({
	...
	validator: Schema.optionalWith(ModelRoute, {
		default: () => ({
			model: "openai-codex/gpt-5.6-sol",
	...
	adversary: Schema.optionalWith(ModelRoute, {
		default: () => ({
			model: "anthropic/claude-fable-5",
			thinking: "high" as const,
		}),
...
const FeatureFlowConfig = Schema.Struct({
	...
	routes: Schema.optionalWith(Routes, {
		default: () => Schema.decodeUnknownSync(Routes)({}),
	}),
	planArtifact: Schema.optionalWith(
		Schema.Struct({
			/** Tailnet-only HTTPS path managed through `tailscale serve`. */
			servePath: Schema.optionalWith(Schema.String, {
	...
	budgets: Schema.optionalWith(
		Schema.Struct({
			implementationMaxTurns: Schema.optionalWith(Schema.Number, {
				default: () => 18,
			}),
			validationMaxTurns: Schema.optionalWith(Schema.Number, {
				default: () => 10,
			}),
			adversaryMaxTurns: Schema.optionalWith(Schema.Number, {
				default: () => 10,
			}),
			spawnTimeoutMs: Schema.optionalWith(Schema.Number, {
				default: () => 900_000,
			}),
			rpcReplyTimeoutMs: Schema.optionalWith(Schema.Number, {
				default: () => 20_000,
			}),
		}),
		{
	...
	archive: Schema.optionalWith(
		Schema.Struct({
			...
			repository: Schema.optionalWith(Schema.String, {
				default: () => "pi-feature-archives",
			}),
			...
			searchRoots: Schema.optionalWith(Schema.Array(Schema.String), {
				default: () => [homedir()],
			}),
			/** Extra context-only files/directories. Supports {featureId} and {workItem}; relative paths use the feature project cwd. */
			extraPaths: Schema.optionalWith(Schema.Array(Schema.String), {
				default: () => [],
			}),
		}),
		{
			default: () => ({
				repository: "pi-feature-archives",
				branch: "main",
				searchRoots: [homedir()],
				extraPaths: [],
			}),
...
export function workerRoute(
	...
	return workerKind === "fable"
		? config.routes.fableWorker
		: config.routes.worker;
}

/** Use a different model family when Fable 5 authored the plan under review. */
export function adversaryRoute(
	config: FeatureFlowConfig,
	activeModel: { provider?: string; id?: string } | null | undefined,
): ModelRoute {
	return isFableFiveModel(activeModel)
		? { ...SOL_HIGH_ADVERSARY_ROUTE }
		: config.routes.adversary;
}

const decodeConfig = Schema.decodeUnknown(Schema.parseJson(FeatureFlowConfig));

export class FeatureConfig extends Effect.Service<FeatureConfig>()(
	"FeatureConfig",
	{
				...
				raw === null
					...
					: yield* decodeConfig(raw).pipe(
							Effect.tapError((error) =>
								Effect.logWarning(
									`feature-flow config.json invalid, using defaults: ${String(error)}`,
								),
							),
