...
import { renderPlanHtml } from "./markdown.ts";
...
export class PlanPublisher extends Effect.Service<PlanPublisher>()(
	"PlanPublisher",
	{
		effect: Effect.gen(function* () {
			...
			const publish = (
				state: FeatureState,
				planRevision: number,
			): Effect.Effect<string, PublishFailed> =>
				Effect.gen(function* () {
					const servePath = normalizedServePath(config.planArtifact.servePath);
					if (!servePath)
						return yield* Effect.fail(
							new PublishFailed({
								reason: `Invalid Tailscale serve path: ${config.planArtifact.servePath}`,
							}),
						);

					const featurePublishedRoot = join(
						featureDir(state.featureId),
						"published",
					);
					const featureServePath = `${servePath}/${state.featureId}`;
					const sourceFileName = `plan-rev${planRevision}.md`;
					const fileName = `plan-rev${planRevision}.html`;
					yield* Effect.tryPromise({
						try: async () => {
							await mkdir(featurePublishedRoot, {
								recursive: true,
								mode: 0o700,
							});
							const sourcePath = artifactPath(state.featureId, "plan");
							const markdown = await readFile(sourcePath, "utf8");
							await copyFile(
								sourcePath,
								join(featurePublishedRoot, sourceFileName),
							);
							await writeFile(
								join(featurePublishedRoot, fileName),
								renderPlanHtml(
									markdown,
									state.title,
									state.workItem.key,
					...
					).pipe(Effect.provideService(PiApi, pi));
					if (serve.code !== 0) {
						return yield* Effect.fail(
							new PublishFailed({
								reason:
					...
					const status = yield* piExec("tailscale", ["status", "--json"], {
						timeout: PUBLISH_TIMEOUT_MS,
					}).pipe(Effect.provideService(PiApi, pi));
					const dnsName =
						status.code === 0 ? tailscaleDnsName(status.stdout) : null;
					if (!dnsName)
						return yield* Effect.fail(
							new PublishFailed({
								reason:
									status.stderr.trim() || "Tailscale DNS name is unavailable.",
							}),
						);
